...
-> Recalcular path
-> Se verificar loop, contorna obstáculo e calcular jps mais à frente
...
class Studentv7(Snake):
	def __init__(self, body=[(0,0)], direction=(1,0), name="SV5"):
	...
	def check_exists_path(self, maze, pos, direct, body, n=0, count_tail=True):
		...
		if all([p[0] for p in positions]):
			return False
		if len([p for p in positions if p[0]==False])>1:
			return True
		else:
			next_p = positions[0][1]
			next_dir = positions[0][2]
			for p in positions:
				if p[0]==False:
					if p[1]!=maze.foodpos:
						body = [p[1]]+body[:-1]
