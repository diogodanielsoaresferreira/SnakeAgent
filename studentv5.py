...
-> jps sempre que comida muda de tile e len(jumps)<=3
-> Verificar se next step não leva a um beco sem saída
-> Recalcular path
-> Se verificar loop, tenta contornar obstáculo e calcular jps mais à frente
...
class Studentv5(Snake):
	def __init__(self, game, body=[(0,0)], direction=(1,0), name="SV5"):
		...
		try:
			...
			if self.jumps==[]:
				if self.loop == False:
					...
					self.direction = self.around_walls(maze, self.direction, [])
					self.loop_count +=1
					self.snake2.last_body = self.snake2.body
					return
	...
	def get_next_dir(self, jump, maze):
		...
		while validdir!=[]:
			...
			if (newpos not in best_head_next_moves or self.points>self.snake2.points) and self.check_exists_path(maze, newpos, next_pos[0], 0, True):
				return next_pos[0]
	...
	def around_walls(self, maze, next_dir, explored_dirs=[]):
		...
		if self.obs_dir==None:
			...
			if self.is_safe_path(maze, next_pos, next_dir):
				# Go Ahead
				if next_pos not in self.snake2.body+self.body+maze.obstacles:
					# Check if is external corner
					if self.add(next_pos,self.obs_dir) not in maze.obstacles:
						if self.obs_dir[0]==0:
							if self.add(next_pos,[1,self.obs_dir[1]]) in maze.obstacles:
								self.obs_dir=(1,self.obs_dir[1])
							else:
								self.obs_dir=(-1,self.obs_dir[1])
						elif self.obs_dir[1]==0:
							if self.add(next_pos,[self.obs_dir[0],1]) in maze.obstacles:
								self.obs_dir=(self.obs_dir[0],1)
							else:
								self.obs_dir=(self.obs_dir[0],-1)
					self.game.paint([self.add(next_pos,self.obs_dir)],(128,128,128))
				...
				# Inside corner
					...
					invaliddir=[x for (x,y) in complement if list(y)==list(self.direction)]
					validdir=[dir for dir in directions if not ( dir in invaliddir ) and self.add(self.body[0],dir) not in maze.obstacles+self.body+self.snake2.body]
					
					for dir in validdir:
						obstacles = [ob_dir for ob_dir in [up, down, left, right] if self.add(self.add(self.body[0],dir),ob_dir) in maze.obstacles]
						
						if len(obstacles)>0:
							self.obs_dir = obstacles[0]
						else:
							self.obs_dir = tuple(self.jps.get_dir(next_pos, self.add(self.body[0],dir)))
			...
			complement=[(up,down),(down,up),(right,left),(left,right)]
			invaliddir=[list(x) for (x,y) in complement if list(y)==list(self.direction)]
	...
	def is_safe_path(self, maze, pos, dir):
		...
		# Check Chicken Condition of if we get trapped
		if (pos not in best_head_next_moves or self.points>self.snake2.points) and self.check_exists_path(maze, pos, dir, 0, False):
			return True
		return False

	# Check if path has an exit
	def check_exists_path(self, maze, pos, direct, n=0, count_tail=True):

		# Stop Condition
		if n>=max(self.mapsize[0], self.mapsize[1]) or n>=10:
			return True
		...
		if count_tail:
			...
			pos = [(self.add(pos, direction) in maze.obstacles+[self.body[0]], self.add(pos, direction), direction) for direction in possible_next_dirs]
		...
		if not any([p[0] for p in pos]):
			...
			for p in pos:
				if p[0]==False:
					return self.check_exists_path(maze, p[1], p[2], n+1, count_tail)
...
class JPS:
	...
	def get_goal(self, pos):
		...
		# If on other tile and gave less than 50 steps, return true if position os on food tile.
		if goal_tile!=snake_tile and self.snake.count_food<50:
