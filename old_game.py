...
class SnakeGame:
    def __init__(self, hor=60, ver=40, tilesize=20, fps=50, visual=False, obstacles=15, mapa=None, timeout=20000):
    ...
    def printstatus(self):
        ...
        if len([p for p in self.players if not p.IsDead]) == 1:
            ...
            dead = "All dead..."
            player = [p for p in self.players]
            if player[0].points > player[1].points:
                print(player[0].name)
            elif player[0].points < player[1].points:
                print(player[1].name)
            else:
                print('draw')
            if self.screen == None:
    ...
    def start(self):
        ...
        while self.screen != None:
            ...
            if event.type == QUIT or (event.type == pygame.KEYDOWN and event.key == K_q): #close window or press Q
                pygame.quit(); 
                exit()
    def paint(self, l, color):
        for elem in l:
            pygame.draw.rect(self.screen, color, (elem[0]*self.tilesize,elem[1]*self.tilesize,self.tilesize,self.tilesize) ,0)